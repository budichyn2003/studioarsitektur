...
export default function ApplyButton({ jobTitle }: { jobTitle: string }) {
  ...
  // States untuk menampung file upload CV & Portfolio
  ...
  const handleApply = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // LOGIC SEMENTARA: Karena belum ada endpoint pengiriman email di backend,
    // Kita jalankan simulasi loading sukses 2 detik sesuai request "Alert Sukses"
    setTimeout(() => {
      setSubmitting(false);
      setSuccess(true);
      setTimeout(() => {
              ...
              <>
                ...
                <form onSubmit={handleApply} className="flex flex-col gap-5">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input required type="text" className="w-full border-b border-gray-300 py-2 focus:outline-none focus:border-black transition-colors text-[14px]" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[11px] text-[#999999] uppercase tracking-widest font-medium">WhatsApp</label>
                    <input required type="tel" className="w-full border-b border-gray-300 py-2 focus:outline-none focus:border-black transition-colors text-[14px]" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[11px] text-[#999999] uppercase tracking-widest font-medium">Email</label>
                    <input required type="email" className="w-full border-b border-gray-300 py-2 focus:outline-none focus:border-black transition-colors text-[14px]" />
                  </div>
                  <div className="flex flex-col gap-1.5 mt-2">
                    <label className="text-[11px] text-[#999999] uppercase tracking-widest font-medium">Upload CV (PDF Max 10MB)</label>
                    <input required type="file" accept=".pdf" onChange={(e) => setCvFile(e.target.files?.[0] || null)} className="text-[13px] file:mr-4 file:py-2 file:px-4 file:rounded-sm file:border-0 file:font-medium file:bg-gray-100 file:text-black hover:file:bg-gray-200 cursor-pointer" />
                  </div>
                  <div className="flex flex-col gap-1.5 mt-2">
                    <label className="text-[11px] text-[#999999] uppercase tracking-widest font-medium">Upload Portfolio (PDF Max 10MB)</label>
                    <input required type="file" accept=".pdf" onChange={(e) => setPortfolioFile(e.target.files?.[0] || null)} className="text-[13px] file:mr-4 file:py-2 file:px-4 file:rounded-sm file:border-0 file:font-medium file:bg-gray-100 file:text-black hover:file:bg-gray-200 cursor-pointer" />
                  </div>
